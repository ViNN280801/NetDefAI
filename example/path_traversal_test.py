...
# Load Path Traversal patterns from the file
try:
    with open("patterns/traversal.txt", "r") as f:
        TRAVERSAL_PATTERNS = [
            line.strip() for line in f if line.strip() and not line.startswith("#")
        ]
    if not TRAVERSAL_PATTERNS:
        print("Warning: No Path Traversal patterns loaded from patterns/traversal.txt")
except FileNotFoundError:
    print("Error: patterns/traversal.txt not found. Please ensure the file exists.")
    TRAVERSAL_PATTERNS = ["../etc/passwd"]  # Default fallback
...
def generate_traversal_event():
    """Generate a security event with Path Traversal payload."""
    source_ip = f"{random.randint(1, 255)}.{random.randint(1, 255)}.{random.randint(1, 255)}.{random.randint(1, 255)}"
    traversal_payload = random.choice(TRAVERSAL_PATTERNS)

    # Inject traversal pattern into a plausible field like 'file', 'path', 'resource'
    payload_key = random.choice(["file", "path", "resource", "page", "include"])

    return {
        "source_ip": source_ip,
        "event_type": "file_access_request",  # Example event type
        "source_type": "Traversal Tester",
        "payload": {
            "user": f"user_{random.randint(100,999)}",
            payload_key: traversal_payload,  # Use the random key here
            "action": "read",
            "timestamp": time.time(),
        },
    }
...
def attack_worker(token, requests_count, sleep_min, sleep_max, worker_id):
    """Worker function that sends Path Traversal requests."""
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

    success = 0
    blocked = 0
    failed = 0  # Other failures

    print(
        f"Worker {worker_id} started - sending {requests_count} Path Traversal requests"
    )

    for i in range(requests_count):
        try:
            event = generate_traversal_event()
            response = requests.post(
...
def main():
    parser = argparse.ArgumentParser(
        description="Path Traversal Test Tool for API Gateway"
    )
    parser.add_argument(
        "-t",
        "--threads",
    ...
    print("=== Path Traversal Test Tool for API Gateway ===")
    print(f"Target: {BASE_URL}")
    print(f"Threads: {args.threads}")
    print(f"Requests per thread: {args.requests}")
    print(f"Total Path Traversal patterns loaded: {len(TRAVERSAL_PATTERNS)}")
    ...
    print("=== Path Traversal Test Results ===")
